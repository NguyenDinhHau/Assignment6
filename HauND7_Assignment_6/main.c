...
uint32_t enterElementOfArr(uint32_t head)
{
    ...
    uint32_t values = 0;
    uint32_t compare = 0;
    printf("\nEnter position of arr from 0 to 19 : ");
    scanf("%d",&position);
    if(position <= 19 && position >= 0)   /* Check position bigger than 0 and less than 20 */
       {
       if(assignment3[position] == 0xff)  /* check value of position */
          {
            printf("\nEnter value of Array from 0 to 100: ");
            scanf("%d",&values);
            if(values  >= 0 && values<= 100 )
            {
               ...
               if(compare != values)
               {
                ...
                head = AddLinkedList(head,position,values);  /* add value to linkedlist when it is ok */
...
uint32_t DelValue(uint32_t head, uint32_t position)
{
     ...
     {
          head = DelHead(head);    /* delete first node of linkedlist */
     }
     else
     {
          ...
          next[current] = next[position];   /* delete node when you known value of node */
          next[position] = 0xff;
     }
...
uint32_t delElementOfArr(uint32_t head)
{
     ...
     if(values <=100 && values>=0)
     {
          for(i=0;i<=19;i++)
          {
               if(assignment3[i]== values)
               {
                    compare = values;   /* Flag to recognize when ass[i] = value*/
                    position = i;
                    break;
               }
               else
...
uint32_t pev = 0;
     ...
     if(head == 0xff)  /* if linked list equal null */
     {
          head = position;
     }
     else
